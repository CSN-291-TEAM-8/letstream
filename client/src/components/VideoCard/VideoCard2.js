...
const VideoCard2 = ({url, _id,title, channel, views, timestamp, likesCount,dislikesCount,description,visibility,noHoverplay,video}) => {
  ...
  if(video){    
      return (
        <div className='videocard2'>
            {show?<video className='videocard_video' src={video.url} muted={true} autoPlay onMouseOut={()=>{setShow(false)}}></video>:<img className="videocard_video" src={video.url.replace(path.extname(video.url),".jpg")} onMouseOver={()=>{window.timeout = setTimeout(()=>{setShow(true)},700)}} onMouseOut={()=>{if(window.timeout){clearTimeout(window.timeout)}}}/>}
            <div className="videocard2_info">
        ...
        <div className='videocard2' key={_id}>
          ...
          <div className="videocard2_info">
            ...
            <div className="videocard_text" >
              ...
              <div className="likeinfo"><div>{likesCount}&nbsp;&nbsp;<ThumbUpIcon/></div>  <div>{dislikesCount}&nbsp;&nbsp;<ThumbDownIcon/></div><div>{visibility==="public"&&<PublicIcon/>}
